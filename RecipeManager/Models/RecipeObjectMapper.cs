...
{
	public class RecipesObjectMapper : SQLiteConnection
	{
		static object lockObject = new object();
		public RecipesObjectMapper (string path) : base(path)
		{
			CreateTable<Recipe>();
		...
		public Ingredient GetIngredient(int recipeId, int id)
		{
			lock (lockObject)
			{
				return Table<Ingredient>().FirstOrDefault(x => x.Id == id && x.RecipeId == recipeId );
			}
		}
